    ...
    <div className="col-12 col-md-4">
      <div className={`card ${plan?.isRecomended ? 'bg-primary text-white' : ''}`}>
        <div className="card-body position-relative">
          {plan?.trialDays > 0 && (
            <div className={`${style.flag}`}>{plan.trialDays} days trial</div>
          )}
          <div className="py-5 px-3 text-center flex-grow-1">
            ...
            >
              {plan?.title || plan?.planName || plan?.name || 'Plan'}
            </div>
            <div
              className={`${
            ...
            <ul className="list-unstyled font-size-18 mb-5 text-left">
              {features.map(feature => (
                <li key={feature?.title || feature}>{feature?.title || feature}</li>
              ))}
            </ul>
            <Link
              to={`/plans/${plan?._id || plan?.id}/edit`}
